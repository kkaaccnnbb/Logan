		   ...
		   for p, I in next, wood:GetChildren() do
            ...
            local ltem = jjj
             ...
             for e = 1, 15 do
               ...
               jjj:PivotTo(game.Workspace["Region_Volcano"].Lava.Lava.CFrame)
                game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(ltem,bypassanticheat)
      ...
      if StealType ~= "TreeClass" then
        if v:FindFirstChild("Type") and tostring(v.Type.Value) == StealType then
          if v.PrimaryPart then
                       game:GetService("ReplicatedStorage").PlaceStructure.ClientPlacedStructure:FireServer(nil,lp.Character.HumanoidRootPart.CFrame,lp, nil, "Broom", true)
...
workspace.PlayerDraggables[""].ArcadeCOPY:SetPrimaryPartCFrame(CFrame.new(89.5275879, 17.3678341, 674.52533))
   elseif list == "连接逻辑店" then
  ...
  if state then
    workspace.ChildAdded:connect(function(Dragger)
      if tostring(Dragger)=="Dragger"then
        ...
        BodyPosition.D=800;
        BodyPosition.maxForce=Vector3.new(1,1,1)*100000000;
        BodyGyro.maxTorque=Vector3.new(1,1,1)*2000000;
        BodyGyro.P=1200;
        BodyGyro.D=140;
  ...
  while wait() do
    if bai.away == true then
      ...
      for _, Log in pairs(game.Workspace.LogModels:GetChildren()) do
        if Log.Name:sub(1, 6) == "Loose_" and Log:findFirstChild("Owner") then
          if Log.Owner.Value == game.Players.LocalPlayer then
              Log.WoodSection.CFrame=lp.Character.HumanoidRootPart.CFrame           
          end
        end
      end
    end
...
bai.away = false
    ...
    Section5:Dropdown("选择树","Dropdown",{"普通树","曲奇树","卡通树","幻影树","砖头木","生命树","玻璃冰木","红颜树", "石头木","沼泽黄金", "樱花", "蓝木", "冰木", "火山木", "橡木", "巧克力木", "白桦木", "黄金木", "雪地松", "僵尸木", "大巧克力树", "椰子树","南瓜木","幽灵木","大理石木","天堂木","虚拟木","玻璃木","糖果树","积木树","发光红色糖果木","彩虹树","雷电木","煤炭木","岩浆树","紫木","下水道木","辐射木","地狱木","沙滩木","白洋木","发光彩虹木","布料木","虚空木","裂纹木","幽灵食尸者","金子木","生锈木","土豆木","星空木","秋天木","星星木","雪木","死木","火焰木","鬼火","彩虹火焰"},function(b)
	if b=="普通树"then
...
sjj = game.Workspace.PlayerModels.ChildAdded:connect(function(v)
...
Sectionautobuy:Button("隐身飞行", function()
  game:GetService("ReplicatedStorage").PlaceStructure.ClientPlacedStructure:FireServer(nil,lp.Character.HumanoidRootPart.CFrame,lp, nil, "Broom", true)
end)

Sectionautobuy:Button("买(请勿使用,后果自负)", function()
    --Fix by 0x15
    local ltem = workspace.Stores.TravelingTrader.ShopItems.UtilitySports
local Counter = ltem.Parent.Parent.Counter
local bypassanticheat = "Ifyouarereadingthisstophackingbrolegitalsokrnlisbadbtw432rewdWdwFe432432rwDWDAVWdawgdGWAYDFGYTUW"
for e = 1, 15 do
    game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(ltem,bypassanticheat)
    ltem:PivotTo(Counter.CFrame)
    game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(ltem,bypassanticheat)
    game:GetService('RunService').Stepped:wait();
end--these by old version bai
wait(0.5)
